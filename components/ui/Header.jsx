      ...
      <div className="relative flex items-center justify-between">
        ...
        >
          ...
          <div className="py-0 sm:py-1 hidden sm:block ">
            <h3 className="my-1 sm:my-2 font-semibold text-left text-base md:text-2xl leading-6  ">
              Reyhan Çetin X
            </h3>
            <p className="mt-1 font-light text-2xs sm:text-base tracking-tight sm:border-t border-indigo">
              Bilinç ve Farkındalık Alanı
            </p>
          </div>
        </Link>
        {/* fixed:takip ediyor;  absolute: sadece üstte kalıyor*/}
        <nav className="absolute py-1 my-1 md:my-2 px-6 border border-solid border-dark rounded-full font-light text-sm md:text-base bg-white bg-opacity-40 top-0 sm:top-2 right-1/2 translate-x-1/2 bg-light/60 backdrop-blur-sm z-50">
          <ul className="flex justify-center items-center space-x-4 tracking-wider ">
            {menu.map((p) => (
              <li
                key={p.id}
                className={`border-b border-b-white hover:border-b-gray-200 hover:text-gray-500 transition-colors `}
              >
                <Link href={`/${p.link}`} title={`${p.hover}`}>
                  {p.title.toLowerCase()}
                </Link>
              </li>
            ))}
